...
static ex rubi141H(ex a, ex j, ex b, ex n, ex p, ex x)
{
        ...
        if ((j*p+1).is_zero())
                return x * Power(a*power(x,j)+b*power(x,n), p) / p / (n-j) / Power((a*power(x,j)+b*power(x,n))/b/power(x,n), p) * _2F1(-p, -p, -p+1, -a/b/power(x,n-j));
        if ((n*p+1).is_zero())
                return x * Power(a*power(x,j)+b*power(x,n), p) / p / (j-n) / Power((a*power(x,j)+b*power(x,n))/a/power(x,j), p) * _2F1(-p, -p, -p+1, -b/a/power(x,j-n));
        return x * Power(a*power(x,j)+b*power(x,n), p) / (j*p+1) / Power((a*power(x,j)+b*power(x,n))/a/power(x,j), p) * _2F1(-p, (j*p+1)/(n-j), (j*p+1)/(n-j)+1, -b/a*power(x,n-j));
}
